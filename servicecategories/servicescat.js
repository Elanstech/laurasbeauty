...
function updateHeroSection(config) {
    ...
    if (categoryDesc)     categoryDesc.textContent     = config.description;
}
...
/**
 * Format a raw price value into a display string.
 * Handles numbers (89 → "$89"), "FREE", and pre-formatted strings ("$89").
 */
function formatPrice(v) {
    ...
    // Already a string like "$89" — return as-is
    return String(v);
}

/**
 * Build the price HTML for a service card or modal.
 * Supports originalPrice (new JSON), regularPrice (legacy JSON), and FREE giveaways.
 */
function buildPriceHTML(service) {
    let html = '';

    // Support both field names
    const original = service.originalPrice ?? service.regularPrice ?? null;
    const current  = service.price ?? null;

    if (original !== null && current !== null) {
        // Show crossed-out original + sale price
        html += `<span class="price-original">${formatPrice(original)}</span> `;
        html += `<span class="price-sale">${formatPrice(current)}</span>`;
    } else if (current !== null) {
        html += `<span>${formatPrice(current)}</span>`;
    }

    // Show badge only when it carries meaningful info (not just "Special")
    if (service.priceLabel && service.priceLabel !== 'Special' && service.priceLabel !== 'Giveaway') {
...
function openModal(service) {
    ...
    // Image
    if (modalImage) {
        modalImage.style.backgroundImage = service.image
            ? `url('${service.image}')`
            : `url('https://images.unsplash.com/photo-1540555700478-4be289fbecef?w=800')`;
    }
    ...
    // Price — show original + sale if available
    if (modalPrice) {
        const priceHTML = buildPriceHTML(service);
    ...
    // Full description
    if (modalFullDescription && modalFullDescriptionWrapper) {
    ...
    // Benefits list
    if (modalBenefitsList && modalBenefitsWrapper) {
...
/**
 * Initialize header scroll + mega menu
 */
function initializeHeader() {
    ...
    document.querySelectorAll('.has-megamenu').forEach(item => {
        ...
        item.addEventListener('mouseleave', function() {
            timeout = setTimeout(() => this.classList.remove('active'), 200);
        });
    });
}
...
function initHeroScrollIndicator() {
    ...
    scrollIndicator.addEventListener('click', () => {
        ...
        const headerHeight = document.querySelector('.premium-header')?.offsetHeight || 0;
        if (target) window.scrollTo({ top: target.offsetTop - headerHeight - 20, behavior: 'smooth' });
    });
}

initHeroScrollIndicator();    nails: {
        jsonFile: '../data/nails.json',
        title: 'Nail Services',
        subtitle: 'Perfect manicures and pedicures for beautiful hands and feet',
        description: 'Pamper yourself with our professional nail care services and stunning nail art.',
